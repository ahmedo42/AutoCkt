...
def create_parser(parser_creator=None):
    ...
    parser.add_argument(
        "--traj_len", type=int, default=30, help="Length of each trajectory"
    )
    return parser
...
def rollout(agent, env_name):
    ...
    while rollout_steps < args.num_val_specs:
        ...
        rollout_steps += 1
        pickle.dump(
            obs_reached, open("{}_obs_reached_test".format(env_config["env"]), "wb")
        )
        pickle.dump(
            obs_nreached, open("{}_obs_nreached_test".format(env_config["env"]), "wb")
        )
